	...
	*/
	function tag_count(stringList) {
		...
		for (var i = 0; i < stringList.length; i++) {
			var isXmlTagStart = stringList[i][0] === "<";
			var isXmlTagEnd = stringList[i][stringList[i].length-1] === ">";

			if (isXmlTagStart && isXmlTagEnd) {
				count++;
			}
		}
		return count;
	...
	*/
	function news_ticker(headlines) {
		...
		for (var i = 0; i < headlines.length; i++) {
			...
			full_headline+=headlines[i]+' ';
			if (full_headline.length + headlines[i].length > 140){
				full_headline+=headlines[i].slice(0,140-full_headline.length);
	...
	*/
	function match_ends(words) {
		...
		for (var i = 0; i < words.length; i++) {
			list=words[i]
			if (list[0]==list[list.length-1] && list.length>=2) {
				count++;
			}
		}
		return count ;
